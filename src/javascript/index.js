...
$(function () {
  ...
  $.each(songs, (index, entry) => {
    $(".music-interface-wrapper").append(
      `<div class="music-interface playlist-order-${index}">
              <div class="album-cover">
                  ...
                  <svg playlist-order="${index}" class="music-play-btn" id="music-interface-${
        index + 1
      }" class="music-play-btn" version="1.1" width="100" height="100" fill="#fff" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" xml:space="preserve"> <path d="M256,0C114.608,0,0,114.608,0,256s114.608,256,256,256s256-114.608,256-256S397.392,0,256,0z M256,496C123.664,496,16,388.336,16,256S123.664,16,256,16s240,107.664,240,240S388.336,496,256,496z"/><path d="M176.416,129.792v252.416L395.024,256L176.416,129.792z M192.416,157.504L363.024,256l-170.608,98.496V157.504z"/></svg>
              ...
              <div class="album-info">
                  ...
                  <a href="${
                    entry.youtube
                  }" target="_blank" class="listen-full">
    ...
    $(entry.handle).on("ended", function () {
      ...
      $(
        `.playlist-order-${index} .music-play-btn, .playlist-order-${index} .music-pause-btn`
      ).toggle();
  ...
  $(`.music-interface svg`).on("click", function () {
    ...
    if ($(`${selector}`).hasClass("playing")) {
      ...
      songs[index].progress = setInterval(function () {
        $(`${selector} .progress-bar`).css(
          "width",
          `${
            (songs[index].handle.currentTime / songs[index].handle.duration) *
            100
          }%`
        );
      }, 100);
      ...
      $.each(songs, (i) => {
        if (index != i && $(`.playlist-order-${i}`).hasClass("playing")) {
          $(
            `.playlist-order-${i} .music-play-btn, .playlist-order-${i} .music-pause-btn`
          ).toggle();
